...
import AnswerWS.IOException_Exception;
import AnswerWS.MalformedURLException_Exception;
...
public class CreateAnswerServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (x < cookies.length) {
            ...
            String result = null;
            try {
                result = createAnswer(accessToken, qid, content);
            } catch (MalformedURLException_Exception | IOException_Exception ex) {
                Logger.getLogger(CreateAnswerServlet.class.getName()).log(Level.SEVERE, null, ex);
            }
    ...
    private String createAnswer(java.lang.String accessToken, int qid, java.lang.String content) throws MalformedURLException_Exception, IOException_Exception {
        // Note that the injected javax.xml.ws.Service reference as well as port objects are not thread safe.
        // If the calling of port operations may lead to race condition some synchronization is required.
        AnswerWS.AnswerWS port = service.getAnswerWSPort();
        return port.createAnswer(accessToken, qid, content);
    }
}
