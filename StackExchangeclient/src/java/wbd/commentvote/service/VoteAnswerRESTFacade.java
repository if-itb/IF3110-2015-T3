...
import wbd.commentvote.controller.VoteAnswerJpaController;
...
public class VoteAnswerRESTFacade {

  private EntityManagerFactory getEntityManagerFactory() throws NamingException {
    return (EntityManagerFactory) new InitialContext().lookup("java:comp/env/persistence-factory");
  }

  private VoteAnswerJpaController getJpaController() {
    try {
      UserTransaction utx = (UserTransaction) new InitialContext().lookup("java:comp/UserTransaction");
      return new VoteAnswerJpaController(utx, getEntityManagerFactory());
    } catch (NamingException ex) {
      throw new RuntimeException(ex);
    }
  }

  public VoteAnswerRESTFacade() {
  }
  ...
  public String count() {
    return String.valueOf(getJpaController().getVoteAnswerCount());
  }
